				...
				<tr>
					...
					<td width="100%" valign="top">
						<table width="100%" >
							...
							db.connect();
							rs=db.execSQL("select g.arms_id,g.weapon1,g.status,g.convict,g.convictdetails,g.on,g.by as user,a.id,a.username from sakansha.arms as g,sakansha.signin as a where g.status=1 and g.by=a.id order by g.on desc");
							while(rs.next())
							{
							...
							db.connect();
							rs=db.execSQL("select g.id,g.owner_name,t.name as user,g.address,a.loc_name from sakansha.hotel as g,sakansha.location as a,sakansha.subject as u,xmltable('$c/all' passing u.details as \"c\" columns name varchar(100) path 'name')as t where g.status=1 and g.area=a.loc_id and g.applied_by=u.subject_id order by g.applied_on desc");
							while(rs.next())
							{
							...
							db.connect();
							rs=db.execSQL("select g.massid ,g.reason ,g.parti ,t.name as user,g.place from sakansha.massmeeting as g,sakansha.subject as u,xmltable('$c/all' passing u.details as \"c\" columns name varchar(100) path 'name')as t where g.status=1 and g.appliedby=u.subject_id order by g.appliedon desc");
							...
							db.connect();
							rs=db.execSQL("select g.loudid as id,g.username,g.reason,t.name as user,g.location from sakansha.loudspeaker as g,sakansha.subject as u,xmltable('$c/all' passing u.details as \"c\" columns name varchar(100) path 'name')as t where g.status=1 and g.appliedby=u.subject_id order by g.appliedon desc");
							while(rs.next())
							{
